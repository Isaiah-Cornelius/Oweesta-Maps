...
function createPopup(currentFeature) {
  ...
  function buildLendingProductsList() {
    if (currentFeature.properties[config.popupLendingProducts].length > 0) {
  ...
  buildOrgLendingInfo();
  buildLendingProductsList();
  buildAssetFundingMinMax();
  buildInvestmentTermsInfoHTML();
  buildSpecialCertsList();
  buildSpecialProjectsList();
